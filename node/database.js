...
method.getUserWithEmailandUser = function (req, cb) {
  ...
  var str = "SELECT * FROM ACCOUNTS WHERE USER = " + input + " OR " + input +
    " = EMAIL";
  console.log(str);
...
method.createAndStoreSecureResetPasswordCode = function (account_id, cb) {
  ...
  var sql = "INSERT INTO PASSWORD_RESET VALUES('" + randomString + "', " +
    account_id + ");";
  console.log(sql);
  this.connection.query(sql, function (err, rows) {
    if (err) {
...
method.registerAccount = function (req, res, cb) {
  ...
  var checkString = "SELECT * FROM ACCOUNTS WHERE USER = " + "'" + user + "'" +
    " OR EMAIL = " + "'" + email + "'";
  ...
  connection.query(checkString, function (err, rows) {
    if (rows.length == 0) {
      ...
      var submitString = "INSERT INTO ACCOUNTS VALUES('" + user + "','" +
        pass + "',NULL,'" + email + "','" + bio + "','" + ext + "');"
      ...
      var connectionString = "SELECT * FROM ACCOUNTS WHERE USER = " + "'" +
        user + "' AND EMAIL ='" + email + "'";
...
method.resetPassword = function (req) {
  ...
  connection.query(sql, function (err, rows) {
    ...
    sql = "UPDATE ACCOUNTS SET PASS = '" + req.body.password +
...
method.submitData = function (req, res, cb) {
  ...
  var submitString = "INSERT INTO RESOURCES VALUES(" + usrID + ",'" + title +
    "','" + coords + "','" + address + "'," + type + ",NULL,'" + desc + "','" +
    city + "','" + country + "','" + ext + "')";
  console.log(submitString);
  connection.query(submitString, function (err, res) {
    console.log("Queried");
  ...
  /*Finds ID number generated by MYSQL to name image file*/
  var connectionString = "SELECT * FROM RESOURCES WHERE TITLE = " + "'" +
    title + "' AND USER_DESCRIPTION ='" + desc + "'";
  console.log(connectionString);
  connection.query(connectionString, function (err, rows) {
    if (err) throw err;
    var id = rows[0].ID;
    ...
    connection.query(connectionString, function (err, rows) {
      ...
      connection.query(sql, function (err, rows_in) {
        ...
        sum(rows_in, 0, 0, function (result) {
          ...
          var sqlString = "SELECT * FROM ACCOUNTS WHERE ID = " +
            data.usrID;
...
method.addRatingDb = function (req, res) {
  ...
  var outSql = "SELECT * FROM RATINGS WHERE USER_ID = " + req.session.mYid +
    " AND RESOURCE_ID = " + req.session.resource_id;
  console.log(outSql);
  var connection = this.connection;
  connection.query(outSql, function (err, rows) {
    if (rows.length > 0) {
      var editSQL = "UPDATE RATINGS SET RATING = " + score +
        " WHERE USER_ID = " + req.session.mYid + " AND RESOURCE_ID = " +
        req.session.resource_id;
      connection.query(editSQL, function (err, rows) {

      });
    } else {
      var sql = "INSERT INTO RATINGS VALUES(" + req.session.resource_id +
        "," + score + ",NULL," + req.session.mYid + ")";
      console.log(sql);
      connection.query(sql, function (err, rows) {
  ...
  if (req.session.loggedIn) {
    ...
    connection.query(queryString, function (err, rows) {
      ...
      if ((rows.length) >= req.session.dbCount + 1) {
        var sqlstring = "SELECT * FROM RATINGS WHERE RESOURCE_ID = " +
          rows[req.session.dbCount].ID;
        console.log(sqlstring);
        connection.query(sqlstring, function (err, rows_rating) {
          sum(rows_rating, 0, 0, function (result) {
            ...
            var dbsqlString =
              "SELECT * FROM ACCOUNTS WHERE ID = " + data.usrID;
...
method.addComment = function (req) {
  ...
  var sql = "INSERT INTO COMMENTS VALUES(" + req.session.resource_id + ",'" +
    comment + "',NULL," + req.session.mYid + ")";
  console.log(sql);
  this.connection.query(sql, function (err, rows) {
    if (err) {
      throw err;
